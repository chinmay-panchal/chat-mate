...
// PATCH STEP 1: Added import for CustomVideoSource
...
public class MethodCallHandlerImpl implements MethodCallHandler, StateProvider {
    ...
    // PATCH STEP 2: Added customVideoSources map
    private final Map<String, CustomVideoSource> customVideoSources = new HashMap<>();
    ...
    public void onMethodCall(MethodCall call, @NonNull Result notSafeResult) {
        ...
        switch (call.method) {
            ...
            // PATCH STEP 3: Three new cases for custom video track support
            case "createCustomVideoTrack": {
                // Called from Dart: VideoShareService._createCustomTrack()
                // Creates a WebRTC VideoSource + VideoTrack backed by our
                // CustomVideoSource, registers it in localTracks so that
                // the standard flutter_webrtc addTrack call can find it.
                String trackId = call.argument("trackId");
                if (trackId == null || trackId.isEmpty()) {
                    result.error("INVALID_ARG", "trackId is required", null);
                    break;
                }
                CustomVideoSource cvs = new CustomVideoSource(mFactory, trackId);
                customVideoSources.put(trackId, cvs);
                synchronized (localTracks) {
                    localTracks.put(trackId, new LocalVideoTrack(cvs.getTrack()));
                }
                // Return the same shape as getUserMedia video track info so that
                // the Dart side can call addTrack(stream, [trackId]) normally.
                Map<String, Object> info = new HashMap<>();
                info.put("id", trackId);
                info.put("kind", "video");
                info.put("label", "video_share");
                info.put("enabled", true);
                info.put("remote", false);
                info.put("readyState", "live");
                result.success(info);
                Log.d("MethodCallHandler", "✅ createCustomVideoTrack: " + trackId);
                break;
            }

            case "pushVideoFrame": {
                // Called ~15 fps from Dart frame pump with decoded RGBA pixels.
                String trackId = call.argument("trackId");
                byte[] rgba = call.argument("rgba");
                int width = call.argument("width");
                int height = call.argument("height");
                CustomVideoSource cvs = customVideoSources.get(trackId);
                if (cvs != null && rgba != null) {
                    cvs.pushRGBAFrame(rgba, width, height);
                }
                result.success(null);
                break;
            }

            case "disposeCustomVideoTrack": {
                // Called from Dart when video share stops.
                String trackId = call.argument("trackId");
                CustomVideoSource cvs = customVideoSources.remove(trackId);
                if (cvs != null) {
                    cvs.dispose();
                }
                synchronized (localTracks) {
                ...
                result.success(null);
                Log.d("MethodCallHandler", "🗑️ disposeCustomVideoTrack: " + trackId);
                break;
            }
