...
/**
 * An copy of ScreenCapturerAndroid to capture the screen content while being
 * aware of device orientation
 */
@TargetApi(21)
public class OrientationAwareScreenCapturer implements VideoCapturer, VideoSink {
    ...
    /**
     * Static reference to the most recently created MediaProjection. Set
     * immediately after getMediaProjection() is called in startCapture(). Used
     * by native code (e.g. MainActivity) for audio capture without any
     * reflection into private fields. Public static so R8 never removes it.
     */
    public static MediaProjection lastCreatedProjection = null;
     ...
     *
     * @param mediaProjectionPermissionResultData the result data of
     * MediaProjection permission activity; the calling app must validate that
     * result code is Activity.RESULT_OK before calling this method.
     * @param mediaProjectionCallback MediaProjection callback to implement
     * application specific logic in events such as when the user revokes a
    ...
    public synchronized void startCapture(
        ...
        // Store statically so MainActivity can access it for audio capture
        // without any reflection into private fields. R8 never removes public statics.
        lastCreatedProjection = mediaProjection;
        ...
        ThreadUtils.invokeAtFrontUninterruptibly(surfaceTextureHelper.getHandler(), new Runnable() {
            ...
            public void run() {
                ...
                if (mediaProjection != null) {
                    ...
                    // Clear static reference when projection is stopped
                    lastCreatedProjection = null;
                }
            }
        });
    }
